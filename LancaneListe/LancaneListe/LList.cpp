...
bool LList::relocate(int id_start, int id_end, int id_after)
{

	LListNode* prom = head; //cuva se element koji je pre id_start da bismo mogli da prelancamo.
	LListNode* prom2 =head;
	LListNode* start=NULL;
	LListNode* end=NULL;
	LListNode* after=NULL;
	if (head != NULL) {
		while (prom2->next != NULL) {
			
				if (prom2->info == id_start)
				{start = prom2;}
				
				if (prom2->info == id_after)
				{after = prom2;}
				
				if (prom2->info == id_end)
				{end = prom2;}

				prom2 = prom2->next;

				if (id_start == head->info) {
					prom = head;
				}

				if (prom2->next->info == id_start) {
					prom = prom2;
				}
				
			}
		}
	if (start != NULL && end != NULL && after != NULL) {
		prom->next = end->next;
		end->next = after->next;
		after->next = start;
		return true;
